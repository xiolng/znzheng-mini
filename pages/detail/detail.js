  ...
  methods: {
    onLoad (options) {
      ...
      if (API.loginsuccess(app)) {
        Net.request({
          url: API.Getuserlikedinfo(that.data.cid, app.globalData.userInfo.openid),
    ...
    getdetails (cid) {
      ...
      Net.request({
        ...
        success: function (res) {
          ...
          var data = parsed_item.text.replace(/\n/g, "");
          let result = app.towxml(data, 'markdown', {
    ...
    zanstart: function () {
      var that = this;
      if (API.loginsuccess({globalData: wx.getStorageSync('userInfo')})) {
        Net.request({
          url: API.PostLike(that.data.item.cid, app.globalData.userInfo.openid),
          success: function (res) {
            var datas = res.data.data;
    ...
    sendcm: function () {
      ...
      if (this.data.cmtext != '' && this.data.cmtext != undefined && this.data.cmtext != null) {
        if (API.loginsuccess(app)) {
          ...
          Net.request({
            url: API.Postcomment(that.data.item.cid, app.globalData.userInfo.nickName, that.data.cmtext, that.data.replaycoid, app.globalData.userInfo.avatarUrl),
            success: function (res) {
    ...
    share: function () {
      ...
      if (API.loginsuccess(app)) {
        wx.navigateTo({
          url: '../share/share?nickName=' + app.globalData.userInfo.nickName + "&thumb=" + that.data.item.thumb + "&title=" + that.data.item.title + "&path=" + that.data.thispath + "&cid=" + that.data.cid,
        })
      } else {
        API.ConfirmAuth();
      }
